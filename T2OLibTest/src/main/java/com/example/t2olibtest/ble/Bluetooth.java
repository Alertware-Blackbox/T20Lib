...
import static com.example.t2olibtest.ApplicationConstants.WristbandConstants.bytesToHex;


public class Bluetooth {
    Activity mCon=null;
    static Attributes _attributes=null;
    ...
    public static Bluetooth getInstance(Activity parent, BleWrapperUiCallbacks callback, Attributes attributes)
    {
        if(mInstance == null)
        {
    ...
    public void startScanning(Activity mCon) {
    ...
    public boolean initialize(Attributes attributes) {
        _attributes=attributes;
    ...
    public String requestCharacteristicValue(BluetoothGattCharacteristic ch) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) return null;
...
//        else {
    ...
    private BluetoothAdapter.LeScanCallback mDeviceFoundCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(final BluetoothDevice device, final int rssi, final byte[] scanRecord) {
            ...
            if(mCon!=null)
            {
                if(WristbandSharePref.getPairedMac(
                        mCon).equalsIgnoreCase(device.getAddress())) {
                    getBeaconInformation(scanRecord, String.valueOf(rssi));
                    WristbandConstants.is_device_available=true;
                    System.out.println("Device con...");
                }
                else {
                   // Toast.makeText(mCon,"Beacon till not in Range!",Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void getBeaconInformation(byte[] scanRecord, String rssi_val)
    {
        int startByte = 2;
        ...
        if (patternFound) {
            ...
            int major = ((int)scanRecord[startByte + 20]& 0xff) * 0x100 + ((int)scanRecord[startByte + 21]& 0xff);
            int minor = ((int)scanRecord[startByte + 22] & 0xff) * 0x100 + ((int)scanRecord[startByte + 23] & 0xff);
            int power =  scanRecord[startByte + 24];
            _attributes.setUUID(uuid.toUpperCase());
            _attributes.setRssi(String.valueOf(rssi_val));
            _attributes.setMajor( get_maj(String.valueOf(major)));
            _attributes.setMinor(get_min(String.valueOf(minor)));
            int txPowerValue=power;
            double distance=0;
            if ( Integer.parseInt(rssi_val) == 0) {
                distance= -1.0;
            }
            double ratio = Integer.parseInt(rssi_val)*1.0/txPowerValue;
            if (ratio < 1.0) {
           ...
           // WristbandConstants.distance_from_bcon= Math.abs(distance);
           // System.out.println("Total Beacon Value--->> "+ Math.abs(distance));

        }
    }

    private String get_maj(String valueOf) {
        String _val="";
    ...
    private String get_min(String valueOf) {
        String _val="";
        if(!valueOf.equals("NA")) {
            _val=valueOf;
        }
